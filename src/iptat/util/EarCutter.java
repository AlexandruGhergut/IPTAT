...
public class EarCutter implements Triangulator {
	...
	private void getTriangles(List<Point2D.Double> points)
	{
		...
		while (n > 2)
		{
			...
			double delta = determinant(pointA,pointB,pointC);
			//pointB is a convex vertex
			if (delta > 0)
			{
				for (int i = 0; i<n-3 && flag==true;i++)
				{
					double a = Math.abs(determinant(pointA,points.get(i),pointB));
					double b = Math.abs(determinant(pointB,points.get(i),pointC));
					double c = Math.abs(determinant(pointC,points.get(i),pointA));
					if (delta == (a + b +c))
					{
